...
export default function QuorumHistoryPage() {
  ...
  const exportToCSV = () => {
    ...
    const rows = filtered
      .map(
        (i) =>
          `${i.id},${i.quorum_type},${i.state},${i.strategy},${i.votes},${i.threshold},${i.created_at}`,
      )
      .join("\n");
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="mt-2 text-slate-400 text-sm max-w-xl">
              Audit the lifecycle of past distributed consensus attempts. Track
              oracle participation, strategies used, and final consensus
              results.
            </p>
          </div>
          <button
          ...
          {[
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <span
                  className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${stat.change.startsWith("+") ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400"}`}
                >
                  {stat.change}
                </span>
              </div>
              ...
              <tbody className="divide-y divide-white/5">
                {isLoading ? (
                  ...
                  <tr>
                    ...
                    >
                      No history matching your search.
                    </td>
                  </tr>
                ) : (
                  filtered.map((item) => (
                        ...
                        <div className="flex items-center gap-2">
                          <div className="h-1.5 w-16 rounded-full bg-slate-800">
                            <div
                              ...
                              style={{
                                width: `${(item.votes / item.threshold) * 100}%`,
                              }}
                            />
                          </div>
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-[10px] font-bold ${
                            item.state === "consensus_achieved"
                              ? "text-emerald-400"
                              : "text-rose-400"
                          }`}
                        >
                          <div
                            className={`h-1 w-1 rounded-full ${item.state === "consensus_achieved" ? "bg-emerald-400" : "bg-rose-400"}`}
                          />
