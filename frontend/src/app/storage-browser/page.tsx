...
import StorageSearchBar, {
  type StorageSearchState,
} from "@/components/StorageSearchBar";
...
export default function StorageBrowserPage() {
  ...
  const [searchState, setSearchState] = useState<StorageSearchState>({
    query: "",
    types: [],
  });
  ...
  const entries: StorageEntry[] = useMemo(() => {
    const all: StorageEntry[] = Object.entries(DEMO_STORAGE).map(
      ([key, value]) => ({
        key,
        value,
    ...
    if (searchState.types.length > 0) {
      filtered = filtered.filter((e) =>
        searchState.types.includes(detectType(e.value)),
      );
    }
      ...
      <div className="border-b border-gray-800 bg-gray-900 px-6 py-5">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-sm text-gray-400">
            Inspect, search, and export contract storage data with type
            detection and diff tracking.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="flex items-start gap-3 p-4 bg-blue-950/20 border border-blue-800/40 rounded-lg">
          ...
          <div className="text-sm">
            ...
            <p className="text-blue-200/70">
              Backend storage API integration pending. Showing mock data for UI
              testing. Real-time polling will fetch from{" "}
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              onClick={() =>
                exportJSON(DEMO_STORAGE, `storage-${Date.now()}.json`)
              }
