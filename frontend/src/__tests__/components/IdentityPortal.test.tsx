...
import IdentityPortal, {
  type IdentityData,
} from "../../components/IdentityPortal";
...
describe("IdentityPortal", () => {
  it("rejects invalid metadata hashes during onboarding", async () => {
    ...
    expect(screen.getByRole("alert")).toHaveTextContent(
      /metadata hash must be a whole number/i,
    );
  ...
  it("rejects empty reputation deltas before calling the handler", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /did:soroban:gabc1234567890/i }),
    );
    ...
    expect(screen.getByRole("alert")).toHaveTextContent(
      /reputation delta must be a non-zero whole number/i,
    );
