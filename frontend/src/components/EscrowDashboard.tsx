...
export type MilestoneStatus =
  "Pending" | "InProgress" | "UnderReview" | "Approved" | "Rejected" | "Paid";
  ...
  return (
    <div className="flex items-center justify-between py-2 px-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
      <div className="flex items-center gap-2 min-w-0">
        ...
        <span
          className={`text-xs font-medium ${milestoneStatusColor(milestone.status)}`}
        >
          {milestone.status}
        </span>
        <span className="text-xs text-slate-500 font-mono">
          {milestone.amount.toLocaleString()} stroops
        </span>
      </div>
    ...
    <div className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <span
            className={`text-xs font-medium px-2 py-0.5 rounded-full ${escrowStatusColor(escrow.status)}`}
          >
            {escrow.status}
          </span>
        </div>
        ...
        <div className="p-4 space-y-4 bg-white dark:bg-slate-900">
          ...
          <div>
            <div className="flex justify-between text-xs text-slate-500 mb-1">
              <span>Progress</span>
              <span>
                {escrow.paidAmount.toLocaleString()} /{" "}
                {escrow.totalAmount.toLocaleString()} stroops
              </span>
            </div>
          ...
          <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
            ...
            {escrow.status === "Active" &&
              (isClient || caller === escrow.freelancer) && (
                <button
...
export default function EscrowDashboard({
  ...
  async function handleInit() {
    ...
    try {
      await apiCall("/initialize", "POST", {
        admin: initAdmin,
        arbiterFeeBps: parseInt(initFeeBps, 10),
      });
  ...
  async function handleCreate() {
    setError(null);
    const amounts = formMilestones
      .split(",")
      .map((s) => Number(s.trim()))
      .filter((n) => !isNaN(n) && n > 0);
    if (amounts.length === 0) {
      setError("Enter comma-separated milestone amounts");
      return;
    }
    try {
  ...
  async function handleResolve(id: number, ruling: number) {
    setError(null);
    try {
      const r = await apiCall(`/escrows/${id}/resolve`, "POST", {
        caller: actionCaller,
        ruling,
      });
      const labels = ["Freelancer Wins", "Client Wins", "Split 50/50"];
      flash(
        `Dispute resolved: ${labels[ruling]} — payout ${r.data.freelancerPayout} stroops`,
      );
      await refresh();
    } catch (e: unknown) {
  ...
  async function handleSubmitMilestone(escrowId: number, milestoneId: number) {
    ...
    try {
      await apiCall(
        `/escrows/${escrowId}/milestones/${milestoneId}/submit`,
        "POST",
        { caller: actionCaller },
      );
  ...
  async function handleApproveMilestone(escrowId: number, milestoneId: number) {
    ...
    try {
      await apiCall(
        `/escrows/${escrowId}/milestones/${milestoneId}/approve`,
        "POST",
        { caller: actionCaller },
      );
  ...
  async function handleRejectMilestone(escrowId: number, milestoneId: number) {
    ...
    try {
      await apiCall(
        `/escrows/${escrowId}/milestones/${milestoneId}/reject`,
        "POST",
        { caller: actionCaller },
      );
  ...
  async function handleReleaseMilestone(escrowId: number, milestoneId: number) {
    ...
    try {
      ...
      flash(
        `Payment of ${r.data.netPayout} stroops released for milestone #${milestoneId}`,
      );
      await refresh();
    } catch (e: unknown) {
  ...
  const milestoneSum = formMilestones
    .split(",")
    .map((s) => Number(s.trim()))
    .filter((n) => !isNaN(n))
    .reduce((a, b) => a + b, 0);
            ...
            <div className="space-y-4 max-w-lg">
              {[
                ...
                },
                {
                  ...
                  value: formFreelancer,
                  set: setFormFreelancer,
                ...
                },
                {
                  ...
                  value: formArbiter,
                  set: setFormArbiter,
                  placeholder: "Dispute arbiter address",
                },
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
                  Milestone amounts (comma-separated stroops)
                </label>
                <input
                  className="w-full text-sm px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder:text-slate-400"
                ...
                />
                <p
                  className={`text-xs mt-1 ${sumsMatch ? "text-green-500" : "text-amber-500"}`}
                >
              ...
              <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg text-xs text-slate-500">
                Arbiter fee: {arbiterFeeBps / 100}% ({arbiterFeeBps} bps)
                deducted from each milestone payment
              </div>
              <button
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  {
                    label: "Total Escrows",
                    value: analytics.totalEscrows,
                    color: "text-slate-900 dark:text-white",
                  },
                  {
                    label: "Active",
                    value: analytics.activeEscrows,
                    color: "text-green-600",
                  },
                  {
                    label: "Completed",
                    value: analytics.completedEscrows,
                    color: "text-blue-600",
                  },
                  {
                    label: "Disputed",
                    value: analytics.disputedEscrows,
                    color: "text-orange-500",
                  },
                  {
                    label: "Cancelled",
                    value: analytics.cancelledEscrows,
                    color: "text-red-500",
                  },
                  {
                    label: "Value Locked",
                    value: `${analytics.totalValueLocked.toLocaleString()} str`,
                    color: "text-violet-600",
                  },
                  {
                    label: "Total Paid Out",
                    value: `${analytics.totalPaidOut.toLocaleString()} str`,
              ...
              {analytics.totalEscrows > 0 && (
                <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-100 dark:border-slate-700">
                  ...
                  <div className="flex h-5 rounded overflow-hidden gap-0.5">
                    {[
                      {
                        count: analytics.activeEscrows,
                        color: "bg-green-400",
                        label: "Active",
                      },
                      {
                        count: analytics.completedEscrows,
                        color: "bg-blue-500",
                        label: "Completed",
                      },
                      {
                        count: analytics.disputedEscrows,
                        color: "bg-orange-400",
                        label: "Disputed",
                      },
                      {
                        count: analytics.cancelledEscrows,
                        color: "bg-red-400",
                        label: "Cancelled",
                      },
                    ]
                      ...
                      .map(({ count, color, label }) => (
                        <div
                          ...
                          style={{
                            width: `${(count / analytics.totalEscrows) * 100}%`,
                          }}
                          title={`${label}: ${count}`}
                        />
                      ))}
                  ...
                  <div className="flex flex-wrap gap-3 mt-2 text-xs text-slate-500">
                    {[
                      ...
                      <span key={label} className="flex items-center gap-1">
                        <span
                          className={`w-2 h-2 ${color} rounded-full inline-block`}
                        />
                        {label}
                      </span>
                    ))}
