...
function ClaimRow({
  ...
  async function process(approve: boolean) {
    setLoading(true);
    try {
      await apiPost(`/api/warranty/claims/${claim.id}/process`, {
        contractId,
        admin: adminAddress,
        approve,
      });
    ...
    <div className="p-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/50">
      <div className="flex items-start justify-between gap-2 mb-1">
        <span className="text-sm font-medium text-slate-900 dark:text-white">
          Claim #{claim.id} — Warranty #{claim.warrantyId}
        </span>
...
function FileClaimForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const d = await apiPost("/api/warranty/claims", {
        contractId,
        claimant: userAddress,
        warrantyId: parseInt(warrantyId, 10),
        description: description.trim(),
      });
      setOk(`Claim #${d.claimId} filed`);
      setDescription("");
      onCreated();
    } catch (e: unknown) {
          ...
          {warranties
            ...
            .map((w) => (
              <option key={w.id} value={w.id}>
                #{w.id} — Product #{w.productId} (expires{" "}
                {tsToDate(w.expiresAt)})
              </option>
            ))}
        </select>
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="p-4 rounded-lg border border-slate-200 dark:border-slate-700">
        <h4 className="font-medium text-slate-900 dark:text-white mb-3 flex items-center gap-2">
          <Package className="w-4 h-4 text-blue-500" /> Register Product
        </h4>
        <div className="space-y-2">
          ...
          <input
            value={pMfr}
            onChange={(e) => setPMfr(e.target.value)}
            placeholder="Manufacturer address (G…)"
            aria-label="Manufacturer address"
            className="w-full px-3 py-2 text-sm rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          <div className="flex gap-2">
            <input
              ...
              aria-label="Warranty duration in days"
            ...
            <button
              ...
              onClick={() =>
                run(async () => {
                  const d = await apiPost("/api/warranty/products", {
                    contractId,
                    admin: adminAddress,
                    name: pName,
                    manufacturer: pMfr,
                    defaultWarrantySecs: parseInt(pDays, 10) * 86400,
                  });
      ...
      <div className="p-4 rounded-lg border border-slate-200 dark:border-slate-700">
        <h4 className="font-medium text-slate-900 dark:text-white mb-3 flex items-center gap-2">
          <Shield className="w-4 h-4 text-green-500" /> Issue Warranty
        </h4>
        <div className="space-y-2">
          <select
            value={wProduct}
            ...
            {products
              ...
              .map((p) => (
                <option key={p.id} value={p.id}>
                  #{p.id} {p.name} ({secsToLabel(p.defaultWarrantySecs)})
                </option>
              ))}
          </select>
          <div className="flex gap-2">
            ...
            />
            <button
              disabled={loading || !wProduct}
              onClick={() =>
                run(async () => {
                  const d = await apiPost("/api/warranty/warranties", {
                    contractId,
                    admin: adminAddress,
                    productId: parseInt(wProduct, 10),
                    owner: wOwner,
                  });
...
export default function WarrantyDashboard({
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [pc, wc, cc] = await Promise.all([
        apiGet("/api/warranty/products/count" as never, { contractId }).catch(
          () => ({ productCount: 0 }),
        ),
        apiGet("/api/warranty/warranties/count" as never, { contractId }).catch(
          () => ({ warrantyCount: 0 }),
        ),
        apiGet("/api/warranty/claims/count" as never, { contractId }).catch(
          () => ({ claimCount: 0 }),
        ),
      ]);
      ...
      const [prods, warrs, clms] = await Promise.all([
        Promise.all(
          Array.from({ length: pc.productCount ?? 0 }, (_, i) =>
            apiGet(`/api/warranty/products/${i + 1}`, { contractId })
              .then((d) => ({
                id: i + 1,
                name: d.product?.name ?? "",
                manufacturer: d.product?.manufacturer ?? "",
                defaultWarrantySecs: d.product?.default_warranty_secs ?? 0,
                isActive: d.product?.is_active ?? false,
              }))
              .catch(() => null),
          ),
        ),
        Promise.all(
          Array.from({ length: wc.warrantyCount ?? 0 }, (_, i) =>
            apiGet(`/api/warranty/warranties/${i + 1}`, { contractId })
              .then((d) => ({
                id: i + 1,
                productId: d.warranty?.product_id ?? 0,
                owner: d.warranty?.owner ?? "",
                issuedAt: d.warranty?.issued_at ?? 0,
                expiresAt: d.warranty?.expires_at ?? 0,
                isActive: d.warranty?.is_active ?? false,
              }))
              .catch(() => null),
          ),
        ),
        Promise.all(
          Array.from({ length: cc.claimCount ?? 0 }, (_, i) =>
            apiGet(`/api/warranty/claims/${i + 1}`, { contractId })
              .then((d) => ({
                id: i + 1,
                warrantyId: d.claim?.warranty_id ?? 0,
                claimant: d.claim?.claimant ?? "",
                description: d.claim?.description ?? "",
                filedAt: d.claim?.filed_at ?? 0,
                status: (["Pending", "Approved", "Rejected"][
                  d.claim?.status ?? 0
                ] ?? "Pending") as Claim["status"],
              }))
              .catch(() => null),
          ),
        ),
  ...
  async function togglePause() {
    ...
    setOk("");
    try {
      await apiPost(paused ? "/api/warranty/unpause" : "/api/warranty/pause", {
        contractId,
        admin: adminAddress,
      });
      setPaused(!paused);
    ...
    <div className="p-6 bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-slate-200 dark:border-slate-800">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Decentralized product warranties with automated claims
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          ...
          <div
            key={label}
            className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 flex items-center gap-3"
          >
            {icon}
            <div>
          ...
          {userAddress && (
            <div className="p-4 rounded-lg border border-slate-200 dark:border-slate-700">
              <h3 className="font-medium text-slate-900 dark:text-white mb-3 flex items-center gap-2">
                <Plus className="w-4 h-4 text-orange-500" /> File a Claim
              </h3>
              <FileClaimForm
      ...
      {tab === "warranties" && (
        <div className="space-y-2">
          {warranties.length === 0 ? (
            <div className="text-center py-8 text-slate-400 text-sm">
              {contractId ? "No warranties issued" : "Enter a contract ID"}
            </div>
          ) : (
            warranties.map((w) => (
              <div
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span
                    className={`text-xs font-semibold px-2 py-0.5 rounded-full ${w.isActive ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}
                  >
                    {w.isActive ? "Active" : "Voided"}
                  </span>
                </div>
                <div className="text-xs text-slate-500 dark:text-slate-400">
                  Issued: {tsToDate(w.issuedAt)} · Expires:{" "}
                  {tsToDate(w.expiresAt)}
                </div>
              </div>
            ))
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 rounded-lg border border-red-200 dark:border-red-800">
            ...
            >
              {paused ? (
                <>
                  <PlayCircle className="w-4 h-4" /> Unpause
                </>
              ) : (
                <>
