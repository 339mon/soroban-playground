...
function Card({
  ...
  return (
    <div
      className={`bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 ${className}`}
    >
      {children}
    </div>
  );
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`px-2 py-0.5 rounded-full text-xs font-semibold ${colors[status] ?? "bg-slate-100 text-slate-600"}`}
    >
      {status}
    </span>
  );
    ...
    <Card>
      ...
      {result != null && (
        <div className="mt-3 flex items-center gap-2 text-sm text-green-600 dark:text-green-400">
          <CheckCircle2 className="w-4 h-4" /> Patent filed — ID:{" "}
      ...
      {patent && (
        <div className="mt-4 space-y-2 text-sm">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs text-slate-500">
            <span>
              Owner:{" "}
              <span className="font-mono">{patent.owner.slice(0, 12)}…</span>
            </span>
            <span>Licenses: {patent.license_count}</span>
            <span>
              Filed: {new Date(patent.filing_date * 1000).toLocaleDateString()}
            </span>
            <span>
              Expires:{" "}
              {new Date(patent.expiry_date * 1000).toLocaleDateString()}
            </span>
          </div>
        </div>
      )}
...
function AdminActions() {
  ...
  return (
    <Card>
      ...
      {msg && (
        <p className="mt-2 text-sm text-green-600 dark:text-green-400">{msg}</p>
      )}
      <div className="mt-4 flex flex-wrap gap-2">
        <Btn
          onClick={() => act(() => activatePatent(Number(patentId), admin))}
          disabled={loading || !admin || !patentId}
        >
          Activate
        </Btn>
        <Btn
          variant="danger"
          onClick={() => act(() => revokePatent(Number(patentId), admin))}
          disabled={loading || !admin || !patentId}
        >
          Revoke
        </Btn>
    ...
    <Card>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <Input
          ...
          type="number"
          value={form.patentId}
          onChange={set("patentId")}
          placeholder="1"
        />
        <Input
          label="Licensee address"
          value={form.licensee}
          onChange={set("licensee")}
          placeholder="G..."
        />
        <label className="flex flex-col gap-1 text-sm">
          <span className="text-slate-500 dark:text-slate-400">
      ...
      {result != null && (
        <p className="mt-3 text-sm text-green-600 dark:text-green-400">
          ...
          License granted — ID: <strong>{result}</strong>
        </p>
      )}
      <div className="mt-4">
        <Btn
...
function DisputePanel() {
  const { fileDispute, resolveDispute, getDispute, loading, error } =
    usePatentRegistry();
  ...
  const submitFile = async () => {
    ...
    const id = await fileDispute({
      claimant: fForm.claimant,
      patentId: Number(fForm.patentId),
      reason: fForm.reason,
    });
  ...
  return (
    <Card>
      ...
      {tab === "file" && (
        <div className="space-y-3">
          <Input
            ...
            value={fForm.claimant}
            onChange={(e) =>
              setFForm((f) => ({ ...f, claimant: e.target.value }))
            }
            placeholder="G..."
          />
          <Input
            label="Patent ID"
            type="number"
            value={fForm.patentId}
            onChange={(e) =>
              setFForm((f) => ({ ...f, patentId: e.target.value }))
            }
            placeholder="1"
          />
          <label className="flex flex-col gap-1 text-sm">
            ...
            <textarea
              value={fForm.reason}
              onChange={(e) =>
                setFForm((f) => ({ ...f, reason: e.target.value }))
              }
              rows={2}
              className="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-3 py-2 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
            />
          ...
          <Btn
            onClick={submitFile}
            disabled={
              loading || !fForm.claimant || !fForm.patentId || !fForm.reason
            }
          >
            File Dispute
          </Btn>
      ...
      {tab === "resolve" && (
        <div className="space-y-3">
          <Input
            label="Admin address"
            value={rForm.admin}
            onChange={(e) => setRForm((f) => ({ ...f, admin: e.target.value }))}
            placeholder="G..."
          />
          <Input
            label="Dispute ID"
            type="number"
            value={rForm.disputeId}
            onChange={(e) =>
              setRForm((f) => ({ ...f, disputeId: e.target.value }))
            }
            placeholder="1"
          />
          <label className="flex flex-col gap-1 text-sm">
            ...
            <textarea
              value={rForm.resolution}
              onChange={(e) =>
                setRForm((f) => ({ ...f, resolution: e.target.value }))
              }
              rows={2}
              className="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-3 py-2 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
            />
          ...
          <Btn
            ...
            disabled={
              loading || !rForm.admin || !rForm.disputeId || !rForm.resolution
            }
          >
            Resolve
          </Btn>
      ...
      {msg && (
        <p className="mt-3 text-sm text-green-600 dark:text-green-400">{msg}</p>
      )}
    </Card>
  );
}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              Soroban-powered IP management on Stellar Testnet
            </p>
          </div>
        </div>
