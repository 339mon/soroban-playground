...
function PropertiesTab({
  ...
  const fetchProperty = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await apiGet(`/api/reit/properties/${id}`, {
        contractId,
        network,
      });
  ...
  function validate() {
    ...
    if (!Number(form.totalShares) || Number(form.totalShares) < 1)
      errs.totalShares = "Must be >= 1";
    if (!Number(form.pricePerShare) || Number(form.pricePerShare) <= 0)
      errs.pricePerShare = "Must be > 0";
    setFormErr(errs);
  ...
  async function deactivate(pid: number) {
    if (!walletAddress) return showToast("Wallet address required", false);
    try {
      await apiPatch(`/api/reit/properties/${pid}/deactivate`, {
        contractId,
        admin: walletAddress,
        network,
      });
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, totalShares: e.target.value }))
                }
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-gray-700 border border-gray-600 rounded px-3 py-1.5 text-sm text-white focus:outline-none focus:border-indigo-500"
              />
              {formErr.pricePerShare && (
                <ErrorMsg msg={formErr.pricePerShare} />
              )}
            </div>
          </div>
          <button
        ...
        >
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <p className="text-xs text-gray-400">
                Price: {p.pricePerShare?.toLocaleString()} stroops/share
              </p>
              <p className="text-xs text-indigo-300">
                Total dividends: {p.totalDividends?.toLocaleString()}
              </p>
            </div>
            <div className="flex flex-col items-end gap-2">
              <span
                className={`text-xs px-2 py-0.5 rounded-full font-medium ${p.isActive ? "bg-green-900/40 text-green-300 border border-green-700" : "bg-red-900/40 text-red-300 border border-red-700"}`}
              >
          ...
          {p.totalShares > 0 && (
            <div className="mt-2">
              <div className="flex justify-between text-xs text-gray-400 mb-1">
                ...
                <span>
                  {Math.round((p.sharesIssued / p.totalShares) * 100)}%
                </span>
              </div>
              <div className="h-1.5 bg-gray-700 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, (p.sharesIssued / p.totalShares) * 100)}%`,
                  }}
                  role="progressbar"
...
function SharesTab({
  ...
  async function fetchHolding() {
    ...
    if (!pid || !lookupInvestor)
      return showToast("Property ID and investor address required", false);
    setLoadingHolding(true);
    try {
      const data = await apiGet("/api/reit/shares/holding", {
        contractId,
        investor: lookupInvestor,
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      if (action === "mint") {
        ...
        showToast(
          `Minted ${shares} shares — cost: ${data.cost?.toLocaleString()} stroops`,
        );
      } else if (action === "burn") {
        ...
        if (!form.toAddress)
          return showToast("Recipient address required", false);
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-lg p-4">
        ...
        <div className="space-y-2">
          <input
            value={lookupInvestor}
            onChange={(e) => setLookupInvestor(e.target.value)}
        ...
        {holding && (
          <div className="mt-3 bg-gray-700/50 rounded p-3 space-y-1">
            <p className="text-sm font-medium text-white">
              {holding.shares?.toLocaleString()} shares held
            </p>
            <p className="text-xs text-gray-400">
              Dividend snapshot:{" "}
              {holding.dividendsClaimedSnapshot?.toLocaleString()}
            </p>
          </div>
        )}
      </div>
...
function DividendsTab({
  ...
  async function checkPending(e: React.FormEvent) {
    ...
    if (!pid || !pendingForm.investor)
      return showToast("Fill all fields", false);
    setCheckingPending(true);
    try {
      const data = await apiGet("/api/reit/dividends/pending", {
        contractId,
        investor: pendingForm.investor,
        propertyId: String(pid),
        network,
      });
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-lg p-4">
        ...
        <form onSubmit={depositDividends} className="space-y-3" noValidate>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setDepositForm((f) => ({ ...f, propertyId: e.target.value }))
                }
                className="w-full bg-gray-700 border border-gray-600 rounded px-3 py-1.5 text-sm text-white focus:outline-none focus:border-indigo-500"
              />
            </div>
          ...
          >
            <TrendingUp size={14} />{" "}
            {depositing ? "Depositing…" : "Deposit Dividends"}
          </button>
        </form>
      </div>
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-lg p-4">
        ...
        <form onSubmit={checkPending} className="space-y-3" noValidate>
          <div>
            ...
            <input
              id="div-chk-inv"
              value={pendingForm.investor}
              onChange={(e) =>
                setPendingForm((f) => ({ ...f, investor: e.target.value }))
              }
              placeholder="G…"
              className="w-full bg-gray-700 border border-gray-600 rounded px-3 py-1.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500"
            />
          ...
          <div className="flex gap-2">
            <input
              type="number"
              min={1}
              value={pendingForm.propertyId}
              onChange={(e) =>
                setPendingForm((f) => ({ ...f, propertyId: e.target.value }))
              }
              placeholder="Property ID"
        ...
        {pending !== null && (
          <div className="mt-3 bg-gray-700/50 rounded p-3">
            ...
            <p className="text-lg font-bold text-green-300">
              {pending.toLocaleString()} stroops
            </p>
          </div>
        )}
      </div>
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-lg p-4">
        ...
        <form onSubmit={claimDividends} className="space-y-3" noValidate>
          <div>
            ...
            <input
              id="div-clm-inv"
              value={claimForm.investor}
              onChange={(e) =>
                setClaimForm((f) => ({ ...f, investor: e.target.value }))
              }
              placeholder="G…"
              className="w-full bg-gray-700 border border-gray-600 rounded px-3 py-1.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500"
            />
          ...
          <div className="flex gap-2">
            <input
              ...
              onChange={(e) =>
                setClaimForm((f) => ({ ...f, propertyId: e.target.value }))
              }
              placeholder="Property ID"
