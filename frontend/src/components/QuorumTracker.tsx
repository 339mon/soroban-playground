...
export type QuorumState =
  "collecting" | "threshold_reached" | "consensus_achieved" | "failed";
  ...
  return (
    <div className="rounded-2xl border border-white/8 bg-slate-950/40 p-5 backdrop-blur-xl">
      ...
      <div className="mb-6 flex items-start justify-between">
        ...
        <div
          className={`flex items-center gap-1.5 rounded-full bg-slate-900/50 px-3 py-1 text-[10px] font-bold border border-white/5 ${color}`}
        >
          {icon}
          {label}
        </div>
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-[9px] text-slate-500">
                        Oracle ID: {vote.oracle_id.slice(0, 8)}...
                      </p>
                    </div>
      ...
      <div className="mt-6 pt-4 border-t border-white/5 flex flex-wrap gap-4 text-[9px] text-slate-500 italic">
        <span>
          Strategy:{" "}
          <span className="text-slate-300 font-medium">
            {quorum.strategy.replace("_", " ")}
          </span>
        </span>
        <span>
          Expires:{" "}
          <span className="text-slate-300 font-medium">
            {new Date(quorum.expires_at).toLocaleString()}
          </span>
        </span>
        <span className="ml-auto">
          Target:{" "}
          <span className="text-slate-300 font-mono">
            {quorum.target_id || "None"}
          </span>
        </span>
      </div>
    </div>
