...
const DataMarketplacePanel: React.FC<DataMarketplacePanelProps> = ({
  ...
  const [tab, setTab] = useState<"marketplace" | "studio" | "analytics">(
    "marketplace",
  );
          ...
          {!selectedDataset || !selectedStats ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                <MetricCard
                  label="Licenses"
                  value={fmtNumber(selectedStats.licenseCount)}
                  icon={<KeyRound size={14} />}
                  accent="text-violet-300"
                />
                <MetricCard
                  label="Active buyers"
                  value={fmtNumber(selectedStats.activeBuyers)}
                  icon={<Users size={14} />}
                  accent="text-emerald-300"
                />
                <MetricCard
                  label="Queries"
                  value={fmtNumber(selectedStats.queriesExecuted)}
                  icon={<Activity size={14} />}
                  accent="text-amber-300"
                />
                <MetricCard
                  label="Revenue"
                  value={fmtNumber(selectedStats.revenue)}
                  icon={<Coins size={14} />}
              ...
              <div className="rounded-3xl border border-white/5 bg-slate-900/40 p-6">
                <h3 className="mb-4 flex items-center gap-2 text-sm font-bold text-white">
                  <ShieldCheck size={14} /> Recent privacy-preserving query
                  receipts
                </h3>
    ...
    <div className="flex items-start justify-between gap-3">
      <div>
        ...
        <p className="text-xs text-slate-400">
          By <span className="font-mono">{shortHash(dataset.provider)}</span> ·{" "}
          {fmtTime(dataset.listedAt)}
        </p>
      </div>
      {dataset.delisted ? (
    ...
    <dl className="mt-3 grid grid-cols-3 gap-2 text-[11px] text-slate-400">
      ...
      <div>
        ...
        <dd className="text-white">
          {Math.round(dataset.licenseSeconds / 86_400)}d
        </dd>
      </div>
    </dl>
...
const DetailPanel: React.FC<{
  ...
  const remaining = license
    ? Math.max(0, license.queriesTotal - license.queriesUsed)
    : 0;
    ...
    <div className="space-y-4 rounded-3xl border border-white/5 bg-slate-900/40 p-5">
      <div>
        ...
        <p className="mt-1 truncate font-mono text-[11px] text-slate-500">
          schema: {dataset.schemaHash}
        </p>
        <p className="truncate font-mono text-[11px] text-slate-500">
          pubkey: {dataset.encryptionPubkey}
        </p>
      </div>
      ...
      {license && (
        <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-3 text-xs text-emerald-200">
          ...
          <p className="text-slate-300">
            {license.queriesUsed}/{license.queriesTotal} queries used · expires{" "}
            {new Date(license.expiresAt * 1000).toLocaleString()}
          </p>
        </div>
      )}
      ...
      {!isOwner && (
        <form
          onSubmit={purchase}
          className="space-y-2 rounded-2xl border border-white/5 bg-slate-950 p-3"
        >
          <label
          ...
          <p className="text-[11px] text-slate-400">
            Cost:{" "}
            {dataset.flatPrice +
              dataset.pricePerQuery * Math.max(0, Number(qty) || 0)}
          </p>
        </form>
      )}
      ...
      {!isOwner && (
        ...
        >
          ...
          <textarea
            ...
            rows={3}
            placeholder={
              canQuery ? "SELECT … " : "Buy a license to run queries"
            }
          ...
          <p className="text-[10px] text-slate-500 leading-relaxed">
            <Lock size={10} className="inline" /> The query never leaves your
            device — only its SHA-256 commitment is recorded on-chain.
          </p>
          {lastCommitment && (
...
const ListDialog: React.FC<{
  ...
  const [busy, setBusy] = useState(false);
  const set = <K extends keyof ListDatasetInput>(
    k: K,
    v: ListDatasetInput[K],
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <Field label="Per query" htmlFor="dm-perquery">
            <input
              ...
              onChange={(e) =>
                set("pricePerQuery", Number(e.target.value) || 0)
              }
              className="w-full rounded-xl border border-white/10 bg-slate-950 px-3 py-2 text-sm text-white"
            />
          </Field>
          <Field label="License (s)" htmlFor="dm-secs">
            <input
              ...
              onChange={(e) =>
                set("licenseSeconds", Number(e.target.value) || 1)
              }
              className="w-full rounded-xl border border-white/10 bg-slate-950 px-3 py-2 text-sm text-white"
            />
          </Field>
