...
function PostCard({
  ...
  return (
    <div
      className={`p-4 rounded-xl border ${post.isPremium ? "border-amber-200 dark:border-amber-800 bg-amber-50/30 dark:bg-amber-900/10" : "border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900"}`}
    >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="flex items-center gap-1 text-xs text-slate-400">
              ...
              {post.tipsCollected.toLocaleString()} str
            </span>
            {post.minTip > 0 && (
              <span className="text-xs text-slate-400">
                min tip: {post.minTip.toLocaleString()}
              </span>
            )}
                ...
                <>
                  ...
                  {showTip && (
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        onClick={() => {
                          onTip(post.id, Number(tipAmount));
                          setShowTip(false);
                        }}
...
function CreatorRow({
  ...
  return (
    <div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
      <span
        className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${rank === 1 ? "bg-amber-400 text-white" : rank === 2 ? "bg-slate-300 text-slate-700" : rank === 3 ? "bg-orange-300 text-white" : "bg-slate-200 dark:bg-slate-700 text-slate-500"}`}
      >
        {rank}
      </span>
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 text-xs text-slate-400 mt-0.5">
          ...
          <span>
            <span className="font-medium text-green-600">
              {analytics.withdrawableEarnings.toLocaleString()}
            </span>{" "}
            str
          </span>
        </div>
...
export default function SocialMediaDashboard({
  ...
  const [tab, setTab] = useState<"feed" | "create" | "analytics" | "creators">(
    "feed",
  );
        ...
        <div className="flex items-center gap-2">
          {globalStats && (
            <span className="hidden sm:flex items-center gap-1 text-xs text-slate-400">
              <Activity className="w-3 h-3" /> {globalStats.totalProfiles}{" "}
              creators · {globalStats.totalPosts} posts
            </span>
          )}
          <button
            onClick={refresh}
        ...
        {!hasProfile && caller && (
          <div className="flex gap-2 w-full">
            ...
            <button
              ...
              className="flex items-center gap-1 bg-violet-600 hover:bg-violet-700 text-white px-3 py-1.5 rounded-lg text-sm transition-colors"
            >
        ...
        {hasProfile && myProfile && (
          <div className="flex items-center gap-3 text-sm">
            ...
            <span className="text-slate-400">
              {myProfile.followers} followers · {myProfile.following} following
            </span>
          </div>
        )}
      </div>
        ...
        <div className="space-y-4 max-w-lg">
          {!hasProfile && (
            <div className="text-sm text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 p-3 rounded-lg">
              Create a profile first to publish posts.
            </div>
          )}
          <div>
          ...
          <div className="flex items-center gap-4">
            <label className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300 cursor-pointer">
              <input
                ...
                onChange={(e) => setPostIsPremium(e.target.checked)}
                className="rounded"
              />
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
              Minimum tip (stroops, 0 = any)
            </label>
            <input
              type="number"
              className="w-full text-sm px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white"
              value={postMinTip}
              onChange={(e) => setPostMinTip(e.target.value)}
            />
          </div>
          <button
            onClick={handleCreatePost}
            disabled={!hasProfile}
            className="flex items-center gap-2 bg-violet-600 hover:bg-violet-700 disabled:opacity-40 text-white px-5 py-2.5 rounded-lg text-sm font-medium transition-colors"
          >
        ...
        <div className="space-y-6">
          {!hasProfile ? (
            <div className="text-center py-12 text-slate-400">
              ...
              <p>Create a profile to see your analytics.</p>
            </div>
          ) : (
            myAnalytics && (
              <>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {[
                    {
                      label: "Posts",
                      value: myAnalytics.postCount,
                      color: "text-slate-900 dark:text-white",
                      icon: <MessageSquarePlus className="w-4 h-4" />,
                    },
                    {
                      label: "Followers",
                      value: myAnalytics.followerCount,
                      color: "text-violet-600",
                      icon: <UserPlus className="w-4 h-4" />,
                    },
                    {
                      label: "Subscribers",
                      value: myAnalytics.subscriberCount,
                      color: "text-amber-500",
                      icon: <Star className="w-4 h-4" />,
                    },
                    {
                      label: "Total Likes",
                      value: myAnalytics.totalLikes,
                      color: "text-red-500",
                      icon: <Heart className="w-4 h-4" />,
                    },
                    {
                      label: "Total Tips",
                      value: `${myAnalytics.totalTips.toLocaleString()} str`,
                      color: "text-green-600",
                      icon: <Coins className="w-4 h-4" />,
                    },
                    {
                      label: "Withdrawable",
                      value: `${myAnalytics.withdrawableEarnings.toLocaleString()} str`,
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700">
                    ...
                    <div className="text-3xl font-bold text-green-600 font-mono mb-4">
                      {myAnalytics.withdrawableEarnings.toLocaleString()} str
                    </div>
                    <button
                      onClick={handleWithdraw}
                      disabled={myAnalytics.withdrawableEarnings === 0}
                      className="flex items-center gap-2 bg-green-600 hover:bg-green-700 disabled:opacity-40 text-white px-4 py-2 rounded-lg text-sm transition-colors"
                    >
                  ...
                  <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <input
                          type="number"
                          className="flex-1 text-sm px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white"
                        ...
                        <button
                          onClick={() =>
                            subTarget && handleSubscribe(subTarget)
                          }
                          className="flex items-center gap-1 bg-amber-500 hover:bg-amber-600 text-white px-3 py-2 rounded-lg text-sm transition-colors"
                        >
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                {[
                  ...
                  [
                    "Total Tips",
                    `${globalStats.totalTips.toLocaleString()} str`,
                  ],
                  ["Subscriptions", globalStats.totalSubscriptions],
                  [
                    "Sub Revenue",
                    `${globalStats.totalSubscriptionRevenue.toLocaleString()} str`,
                  ],
                ].map(([label, val]) => (
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {(
              ...
              <button
                ...
                className={`text-xs px-3 py-1 rounded-full transition-colors ${leaderboardBy === by ? "bg-violet-600 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-slate-700"}`}
              >
