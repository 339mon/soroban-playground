...
const DataTypeFormatter: React.FC<DataTypeFormatterProps> = ({
  ...
  return (
    <span
      className={`inline-flex items-start gap-1.5 font-mono text-xs ${className}`}
    >
      {showBadge && (
      ...
      {isMultiline ? (
        <pre className={`whitespace-pre-wrap break-all ${TYPE_STYLE[type]}`}>
          {display}
        </pre>
      ) : (
