...
Both hooks track whether a close is _intentional_ (triggered by the hook's
own cleanup logic) or _unexpected_ (network drop, server restart, etc.).
...
`onerror`, `onmessage`) on the _old_ socket before creating a new one
...
const { data, isConnected, error, subscribe, unsubscribe, send } =
  useWebSocket();
