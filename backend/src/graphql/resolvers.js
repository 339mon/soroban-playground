...
export const resolvers = {
  ...
  Query: {
    projects: checkGraphQLPermission('project:read')(
    ...
    project: checkGraphQLPermission('project:read')(
      async (_parent, { id }, context) => {
        ...
        if (
          ...
          project.creator_id !== context.user.id
        ) {
  ...
  Mutation: {
    createProject: checkGraphQLPermission('project:create')(
    ...
    updateProject: checkGraphQLPermission('project:update')(
      async (_parent, args, context) => {
        const db = getDatabase();
        const project = await db.get('SELECT * FROM projects WHERE id = ?', [
          args.id,
        ...
        const updatedTitle =
          args.title !== undefined ? args.title : project.title;
        const updatedDesc =
          args.description !== undefined
            ? args.description
            : project.description;
        const updatedCat =
          args.category !== undefined ? args.category : project.category;
        const updatedStatus =
          args.status !== undefined ? args.status : project.status;
        const updatedGoal =
          args.funding_goal !== undefined
            ? args.funding_goal
            : project.funding_goal;
        const updatedTags =
          args.tags !== undefined ? JSON.stringify(args.tags) : project.tags;
    ...
    deleteProject: checkGraphQLPermission('project:delete')(
      async (_parent, { id }, context) => {
        const db = getDatabase();
        const project = await db.get('SELECT * FROM projects WHERE id = ?', [
          id,
