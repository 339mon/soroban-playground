...
impl Erc721 {
    ...
    pub fn total_supply(env: Env) -> Result<u64, Error> {
        ...
        Ok(env.storage().instance().get(&TOTAL_SUPPLY).unwrap_or(0))
    }
    ...
    pub fn mint(env: Env, to: Address, token_id: u64) -> Result<(), Error> {
        ...
        env.events()
            .publish((symbol_short!("Transfer"),), (admin, to, token_id));
    ...
    pub fn approve(env: Env, caller: Address, to: Address, token_id: u64) -> Result<(), Error> {
        ensure_initialized(&env)?;
        caller.require_auth();
        let owner = Self::owner_of(env.clone(), token_id)?;
    ...
    pub fn get_royalty(env: Env) -> Result<Option<RoyaltyInfo>, Error> {
        ensure_initialized(&env)?;
        Ok(env.storage().instance().get(&ROYALTY))
    }
}
