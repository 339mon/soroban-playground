...
const sampleQuery = {
  filter: {
    contractId: { $eq: "C123..." },
    $or: [{ amount: { $gte: 1000 } }, { type: { $in: ["swap", "liquidity"] } }],
  },
...
const hasSequentialParams =
  result.sql.includes("$1") && result.sql.includes(`$${result.params.length}`);
console.log(
  `Uses $1 through $${result.params.length}: ${hasSequentialParams ? "✅ Yes" : "❌ No"}`,
);
